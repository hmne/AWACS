...
lbl() {  # sets LBL to the label for key $1 in the current UI language
  if [[ $UI == ar ]]; then
    case $1 in
      ...
      nowifi)    LBL='لا كرت واي فاي في هذا الجهاز (%s) — شغّل اللوحة على الجهاز الذي يعمل عليه awacs.sh' ;;
    ...
    case $1 in
      ...
      nowifi)    LBL='no wireless interface on this machine (%s) - run the dashboard on the device awacs.sh supervises' ;;
...
# ------------------------------- sources ---------------------------------------
read_conf() {  # LOG_FILE / RUN_DIR / SITE_URL / SITE_API from the conf — plain KEY=value only
  [[ -r $CONF ]] || return 0
  local k v
  while IFS='=' read -r k v; do
    ...
    case $k in
      ...
      SITE_URL) SITE_URL=$v ;;
      SITE_API) [[ $v =~ ^[A-Za-z0-9_][A-Za-z0-9_./-]{0,63}$ && $v != *..* ]] && SITE_API=$v ;;
    esac
  done < <(grep -E '^[[:space:]]*(LOG_FILE|RUN_DIR|SITE_URL|SITE_API)[[:space:]]*=' "$CONF" 2>/dev/null)
}
...
have_net() {  # awacs.sh's ladder; the site rung only when SITE_URL is set
  ...
  [[ $(curl -s -o /dev/null -w '%{http_code}' --max-time 3 --data-urlencode "probe=1" \
       "${SITE_URL%/}/${DEVID}/${SITE_API}" 2>/dev/null) == "400" ]]
}
...
main() {
  ...
  # a PC or a WSL shell has no radio: every panel would show dashes — say why, up front
  if [[ ! -d /sys/class/net/$IF/phy80211 ]]; then
    lbl nowifi; NOTICE="${LBL//%s/$IF}${NOTICE:+ | $NOTICE}"
  fi

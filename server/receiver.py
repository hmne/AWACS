#!/usr/bin/env python3
"""receiver.py - minimal self-hosted receiver for the AWACS device_api contract.
...
The device (awacs.sh) posts to  <SITE_URL>/<DEVICE_ID>/device_api.php . This server
...
DEVICE_PATH = re.compile(r"^/([A-Za-z0-9_-]{1,32})/device_api\.php$")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="AWACS device_api receiver (stdlib only)")
    ...
    print(f"awacs receiver on http://{args.host}:{args.port}/<DEVICE_ID>/device_api.php "

...
/**
 * receiver.php — minimal self-hosted receiver for the AWACS device_api contract.
 *
 * PHP 7.4 or newer. One file, no dependencies, no database.
 *
 * Two ways to host it:
 *
 *   1. Behind Apache or nginx: copy it as  <docroot>/<DEVICE_ID>/device_api.php .
 ...
 *      The script acts as the router: it answers  /<DEVICE_ID>/device_api.php  for any
...
/**
 * Where this device's files live: beside the script under Apache/nginx, or
 * data/<id>/ when the script is the router of PHP's built-in server. Null = not a
 * device path (only possible in router mode).
 */
function resolveDataDir(): ?string
{
    ...
    if (!is_string($path) || !preg_match('#^/([A-Za-z0-9_-]{1,32})/device_api\.php$#', $path, $m)) {
        return null;
    }

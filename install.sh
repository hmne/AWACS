...
declare -A MSG_EN=(
  ...
  [loglang_ask]="Language of the local log's story lines (DEBUG diagnostics stay English)"
  [sitelang_ask]="Language of the lines sent to the site"
  [lang_en]="English"
  [lang_ar]="Arabic"
  [site_required]="--log {1} needs --site URL."
  [site_ask]="Site base URL. The script posts to <URL>/<device id>/<endpoint> (receiver.php unless --api says otherwise)"
  ...
  [site_fail]="Expected HTTP 400, got '{1}'. The site is unreachable or does not speak the endpoint contract."
...
declare -A MSG_AR=(
  ...
  [loglang_ask]="لغة أسطر القصة في اللوق المحلي (سطور DEBUG التشخيصية تبقى بالإنجليزية)"
  [sitelang_ask]="لغة الأسطر المرسلة إلى الموقع"
  [lang_en]="الإنجليزية"
  [lang_ar]="العربية"
  [site_required]="الخيار --log {1} يحتاج --site مع العنوان."
  [site_ask]="عنوان الموقع الأساسي. يرسل السكربت إلى <URL>/<معرّف الجهاز>/<نقطة الاستقبال> (receiver.php ما لم يحدد --api غيره)"
  ...
  [site_fail]="المتوقع HTTP 400 لكن الردّ كان '{1}'. الموقع غير متاح أو لا يتبع عقد نقطة الاستقبال."
...
valid_api() { [[ $1 =~ ^[A-Za-z0-9_][A-Za-z0-9_./-]{0,63}$ && $1 != *..* ]]; }   # awacs.sh's SITE_API shape check
valid_lang() { [[ $1 =~ ^(en|ar)$ ]]; }
api_name()  { printf '%s' "${SITE_API:-${EX_SITE_API:-receiver.php}}"; }
...
site_signature() {  # the contract probe: a reachable site answers 400 "Error: no operation"
  http_code "${1}/${DEVICE_ID}/$(api_name)" --data-urlencode "probe=1"
}
ask_site_url() {
  ...
  while :; do
    ...
    say "$(msg site_check "${ans}/${DEVICE_ID}/$(api_name)")"
...
step_log_target() {
  ...
  # languages: the local log always has one; the site's only matters when lines go there.
  # Defaults follow the wizard's own language; a re-run offers the existing conf's values.
  [[ -n $LOG_LANG ]]  || LOG_LANG=$EX_LOG_LANG
  [[ -n $SITE_LANG ]] || SITE_LANG=$EX_SITE_LANG
  if [[ -z $LOG_LANG ]] && (( ! ASSUME_YES )); then
    LOG_LANG=$(ui_menu "$(t log_title)" "$(t loglang_ask)" "${UI_LANG:-en}" en "$(t lang_en)" ar "$(t lang_ar)")
  fi
  if [[ $LOG_TARGET != local && -z $SITE_LANG ]] && (( ! ASSUME_YES )); then
    SITE_LANG=$(ui_menu "$(t log_title)" "$(t sitelang_ask)" "${LOG_LANG:-${UI_LANG:-en}}" en "$(t lang_en)" ar "$(t lang_ar)")
  fi
  ...
  if [[ $LOG_TARGET != local ]]; then
    if [[ -n $SITE_URL ]] && (( ASSUME_YES )); then
      ...
      say "$(msg site_check "${SITE_URL}/${DEVICE_ID}/$(api_name)")"
  ...
  say "LOG_TARGET=${LOG_TARGET}  SITE_URL=${SITE_URL:-(empty)}  SITE_API=${SITE_API:-(receiver.php)}  PROBE_URL=${PROBE_URL:-(default)}  SITE_TZ=${SITE_TZ:-(device)}  LOG_LANG=${LOG_LANG:-(en)}  SITE_LANG=${SITE_LANG:-(en)}"
}
...
render_conf() {  # prints the new conf: existing lines kept, managed keys replaced
  ...
  [[ -n $SITE_LANG ]]   && printf 'SITE_LANG="%s"\n' "$SITE_LANG"
  for i in "${!SAFETY_SSIDS[@]}"; do
...
Options / الخيارات:
  ...
  --log local|both|remote  log target / وجهة اللوق (both and remote need --site under --yes)
  ...
  --api NAME               endpoint file name under <site>/<device id>/, default receiver.php / اسم ملف نقطة الاستقبال
  --log-lang en|ar         language of the local log's story lines / لغة اللوق المحلي
  --site-lang en|ar        language of the lines sent to the site / لغة أسطر الموقع
...
parse_args() {
  ...
  while (( $# )); do
    ...
    case $a in
      --lang|--device-id|--log|--site|--probe|--report-wifi|--tz|--api|--log-lang|--site-lang|--safety|--service|--release-url)
  ...
  if [[ -n $SITE_API ]] && ! valid_api "$SITE_API"; then die "--api must be a plain file name or relative path (letters, digits, _ . / -), no .."; fi
  if [[ -n $LOG_LANG ]] && ! valid_lang "$LOG_LANG"; then die "--log-lang must be en or ar"; fi
  if [[ -n $SITE_LANG ]] && ! valid_lang "$SITE_LANG"; then die "--site-lang must be en or ar"; fi
